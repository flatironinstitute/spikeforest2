...
def main():
    ...
    times = detect_on_channel(sig, detect_threshold=noise_level*detect_threshold, detect_interval=detect_interval, detect_sign=detect_sign, margin=1000)
    times = times[0:num_events]

    snippets = extract_snippets(X, reference_frames=times, snippet_len=snippet_len)
    tt = np.linspace(-1, 1, snippets.shape[2])
    ...
    A = snippets.reshape(snippets.shape[0], snippets.shape[1] * snippets.shape[2])

    A_denoised = knn_denoise(A, k=20, ncomp=4)
    snippets_denoised = A_denoised.reshape(snippets.shape)
