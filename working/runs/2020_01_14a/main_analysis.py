        ...
        for sorter in spike_sorters:
            for recording in recordings:
                if recording['study_set'] in sorter['studysets']:
                    ...
                    if algorithm in ['ironclust', 'tridesclous']:
                        gpu = True
                        jh = job_handler_gpu
...
def _parse_spikeforest_runtime(txt):
    # Example: "#SF-SORTER-RUNTIME#11.459#"
    try:
        lines = txt.splitlines()
        for line in lines:
            if '#SF-SORTER-RUNTIME#' in line:
                i = line.index('#SF-SORTER-RUNTIME#')
                b = line[i:].split('#')
                if len(b) >= 3:
                    return float(b[2])
    except:
